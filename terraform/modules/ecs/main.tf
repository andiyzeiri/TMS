  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        ...
        {
          name      = "QBO_CLIENT_ID"
          valueFrom = var.qbo_client_id_secret_arn
        },
        {
          name      = "QBO_CLIENT_SECRET"
          valueFrom = var.qbo_client_secret_arn
        }
      ]
      ...
      healthCheck = {
        command = [
          "CMD-SHELL",
          "curl -f http://localhost:${var.app_port}/healthz || exit 1"
        ]
        interval    = 30
        timeout     = 5
        retries     = 3
...
resource "aws_appautoscaling_policy" "ecs_policy_cpu" {
  name               = "${var.name_prefix}-${var.environment}-cpu-scaling"
...
resource "aws_appautoscaling_policy" "ecs_policy_memory" {
  name               = "${var.name_prefix}-${var.environment}-memory-scaling"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_target.service_namespace
